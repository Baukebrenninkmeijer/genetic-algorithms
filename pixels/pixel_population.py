...
class Population:
    def __init__(self, target: np.ndarray, popsize: int = 20, mutation_delta: float = 0.1, mutation_prob: float = 0.1, sample_top_n = 0.1, copy_top_perc=0.02,
        ...
        self.pop = [PixelIndividual(shape=self.target.shape, mutate_change=mutation_delta, mutate_prob=mutation_prob) for _ in range(popsize)]
    ...
    def optimize(self, epochs: int, plot_frequency: int = 20, name: str = 'default', plot: bool = False, show: bool = False):
        ...
        for i in (pbar:= tqdm(range(epochs))):
            ...
            # Logging and viz.
            pbar.set_postfix({'Max': f'{max_fitness:.1f}', 'Min': f'{min_fitness:.3f}', 'current': f'{current_best_fitness:.3f}'})
            pbar.refresh()
